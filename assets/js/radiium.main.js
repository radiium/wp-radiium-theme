jQuery( document ).ready( function( $ ) {
	...
	// Gallery scroll left button
	$(".prevBtn").click(function() {
		var scrollLeft = $('.singlePostContainer').scrollLeft();
		var winWidth = $(window).width();
		var scroll = 0;
		if (scrollLeft > 0) {
			if (winWidth > 768) {
				scroll = scrollLeft - (winWidth - 100);
			} else {
				scroll =  scrollLeft - (winWidth - 100);
			}
			if (scroll < 0) {
			...
			$('.singlePostContainer').animate({ scrollLeft: scroll }, 'slow');
		}
    });


	// Gallery scroll right button
	$(".nextBtn").click(function() {
		var scrollLeft = $('.singlePostContainer').scrollLeft();
		var contWidth = document.getElementsByClassName('singlePostContainer')[0].scrollWidth;
		var winWidth = $(window).width();
		var scroll = 0;

		if (scrollLeft < contWidth) {
			if (winWidth > 768) {
				scroll = scrollLeft + (winWidth - 100);
			} else {
				scroll =  scrollLeft + (winWidth - 100);
	...
	$('.galItem').hover(function () {
		if ( $(window).width() > 480) {
			var $item = $(this).find('.galItemInfos');
			if ($item.text()) {
	...
	initPhotoSwipeFromDOM('.postContent');
